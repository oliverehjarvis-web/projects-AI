-- Initial schema: projects, chats, messages, quick_actions, global_context.
--
-- Extracted from the original `_CREATE_SQL` string in db.py. Kept idempotent with
-- IF NOT EXISTS so an existing volume re-applying this migration is a no-op
-- (the migrator only runs unapplied versions, but defence-in-depth doesn't hurt).
